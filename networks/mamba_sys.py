...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, depths=[2, 2, 9, 2],
        ...
        self.norm_up = norm_layer(48)

        if self.final_upsample == "expand_first":
            # print("---final upsample expand_first---")
            self.up = FinalPatchExpand_X2(dim=48)
            self.output = nn.Conv2d(in_channels=48, out_channels=self.num_classes, kernel_size=1,
                                    bias=False)

        ## depth output
        self.convs = nn.ModuleList()
        self.scales = 4
        self.num_ch_dec = [1, 48, 96, 192]
        for s in range(self.scales):
            self.convs.append(Conv3x3(self.num_ch_dec[s], self.num_classes))
    ...
    def depth_forward_up_features(self, x, x_downsample):
        self.outputs = {}
        ...
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                x = self.concat_back_dim[inx](x)
                x = layer_up(x)
                self.outputs[("disp", self.scales - inx)] = self.sigmoid(
                    self.convs[self.scales - inx](x.permute(0, 3, 1, 2)))

        x = self.norm_up(x)  # B H W C
        x = self.up_x2(x)
        self.outputs[("disp", 0)] = self.sigmoid(self.convs[0](x))
        return self.outputs
